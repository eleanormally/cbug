...
func readInternal(filename string, inStruct any, defaultFileValue string) error {
	execLoc, err := os.Executable()
	ifErr(err, "Error getting location of cbug: ", true)
	path := filepath.Dir(execLoc) + "/../" + filename
	if _, err = os.Stat(path); errors.Is(err, os.ErrNotExist) {
		if defaultFileValue != "" {
			os.WriteFile(path, []byte(defaultFileValue), 0644)
		} else {
			return errors.New("CRITICAL ERROR: " + filename + " not found")
		}
	}
	file, err := ioutil.ReadFile(path)
	_ = json.Unmarshal([]byte(file), inStruct)
	return nil
}

func mainOld() {
	conf := configStruct{}
	readInternal("config.json", &conf, `{"containerName": "cbug","exitBehaviourDefault": "shutdown"}`)
	//arg parsing
	...
	readInternal("release-info.json", &releaseInfo, "")
